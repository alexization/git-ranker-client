...
5. [next.config.ts](next.config.ts), [tsconfig.json](tsconfig.json), [eslint.config.mjs](eslint.config.mjs), [Dockerfile](Dockerfile), [docker-compose.yml](docker-compose.yml)에서 build/runtime config surface를 확인한다.
...
`.claude/skills/`는 ECC(MIT — [affaan-m/ECC](https://github.com/affaan-m/ECC))에서 선별한 지식 스킬로 구성된다: `react-patterns`(React 19 훅·서버/클라이언트 경계), `frontend-patterns`(Zustand·React Query), `react-performance`(성능 규칙), `nextjs-turbopack`(Next.js 16), `frontend-a11y`(접근성). 해당 영역 작업 시 자동 참조된다. 테스트 스위트가 없어 testing 계열 스킬은 두지 않았다(도입 시 `react-testing`/`e2e-testing` 추가 검토).
