...
export function UserDetailModal({ username, open, onOpenChange }: UserDetailModalProps) {
  ...
  return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        {/* [Design] 모달 크기 확장 (max-w-xl) 및 둥근 모서리 강화 */}
        <DialogContent
            className="sm:max-w-xl p-0 overflow-hidden bg-background border-none shadow-2xl rounded-[32px] ring-1 ring-black/5 dark:ring-white/10"
        >
          ...
          {isLoading ? (
              ...
              <div className="flex flex-col max-h-[85vh] overflow-y-auto scrollbar-hide">
                ...
                {/* 1. Header Section */}
                <div className="relative pt-12 pb-8 px-6 flex flex-col items-center text-center bg-gradient-to-b from-secondary/50 to-background">
                  <motion.div
                ...
                {/* 2. Stats Grid (New Component) */}
                <div className="px-6 pb-4">
                  <ActivityGrid user={user} />
                </div>

                {/* 3. Footer Actions (Enhanced) */}
                <div className="p-6 pt-2 grid grid-cols-2 gap-3 sticky bottom-0 bg-background/80 backdrop-blur-xl border-t border-border/50">
                  <Button
                      asChild
